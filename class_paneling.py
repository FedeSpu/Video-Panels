...
class DummyClass:
    def __init__(self, panel_width, panel_height, border_px):
        ...
        self.verbose = False
        self.plot_video = False
        
    def plot_images_grid(self, frames, video_path):
        """
    ...
    def stack_frames_grid(self, video):
        """
        ...
        Requires the following class attributes:
            - self.panel_width (int): number of frames per row.
                alpha in the paper.
            - self.panel_height (int): number of frames per column.
                beta in the paper.
            - self.border_px (int): border size in pixels (around frames).
                Suggested value: 0
            - self.fps_limit (int): how many fps between frames to sample
        ...
        Raises:
            AssertionError: If the total number of frames (N) is not perfectly divisible 
                by the number of panels per grid (self.panel_width * self.panel_height).
        """
    ...
    def load_video(self, video_path, C):
        '''
        ...
        Requires the following class attributes:
            ...
            - self.border_px (int): border size in pixels (around frames).  
                Suggested value: 0
            - self.fps_limit (int): how many fps between frames to sample
            - self.verbose (bool): whether to print verbose output
            - self.plot_video (bool): whether to plot the video frames
        ...
        C = C * self.panel_width * self.panel_height
        
        # Compute the offset for frame sampling (gamma in the paper)
        offset = self.fps_limit * vr.get_avg_fps()
                
        # Check if there are enough frames, spaced at least "offset" frames apart
        if (D > offset * C) and (self.panel_width * self.panel_height > 1):
            '''PANELING'''
            ...
            spare_frames = self.stack_frames_grid(spare_frames)

            # For debugging purposes
            if self.verbose:
                print('Paneled:', spare_frames.shape, self.panel_width, self.panel_height, D, vr.get_avg_fps(), D / vr.get_avg_fps(), C)
        else:
            '''NORMAL SAMPLING'''
            # Uniformly sample C frames, from start to end of the video
            # First compute the frame indices, then sample the frames
        ...
        if self.plot_video:
            self.plot_images_grid(spare_frames, video_path)
